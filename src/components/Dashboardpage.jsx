...
const Dashboardpage = () => {
  ...
  useEffect(() => {
    if (!isAuthorized) {
      navigate("/authorize"); // Redirect to authorization page if not authorized
    } else {
      const storeOrgId = sessionStorage.getItem("orgId");

      console.log("Stored Organization ID:", storeOrgId); // Log the storedOrgId
  ...
  }, [apiKey, apiToken, fetchBoardsList, orgId, isAuthorized, navigate]);
    ...
    try {
      ...
      setBoards((prevBoards) =>
        prevBoards.map((board) =>
          board.id === boardId ? { ...board, name: updatedBoard } : board
        )
      );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-teal-950 to bg-cyan-800 p-20">
      ...
      <h3 className="mb-10 text-green-400 text-xl font-bold mt-10">
        Create new board{" "}
      </h3>
      <div className="flex gap-4 mb-4 items-center">
        <button
          className="bg-blue-500 text-white px-4 py-4 rounded-md flex items-center gap-2 hover:bg-blue-600 active:bg-blue-700 transition-colors font-semibold"
      ...
      <h4 className="font-bold text-green-400 mt-10 mb-10 text-xl">
        {" "}
        Boards List
      </h4>
      <div className="flex p-4 my-3 bg-white border border-gray-200 rounded-3xl">
        <ul>
          {boards.map((board) => (
            <li
              key={board.id}
              className="bg-gradient-to-r from-purple-600 via-blue-400 to-pink-600 p-4 my-3 rounded-md shadow-md"
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center space-x-2">
                  <button
                    className="bg-green-400 hover:bg-green-600 active:bg-green-600  text-black px-3 py-2 rounded-md"
                  ...
                  <button
                    className="bg-red-600 hover:bg-orange-600 active:bg-orange-400 text-white px-3 py-2 rounded-md"
              ...
              {isUpdating && selectedBoard === board.id && (
                <div className="mt-2 ml-20">
                  <input
                    ...
                    type="text"
                    value={updatedBoard}
                    onChange={(e) => setUpdatedBoard(e.target.value)}
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
              ...
              {!isUpdating[board.id] && (
                <button
                  className="flex space-x-2 items-center bg-yellow-300 hover:bg-yellow-400 active:bg-yellow-600 text-black px-6 py-2 p-8 rounded-md mt-auto mx-auto  font-bold"
                  onClick={() => {
