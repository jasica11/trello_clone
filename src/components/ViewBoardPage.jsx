    ...
    <>
      <div className="min-h-screen bg-white p-20">
        <h1 className="text-3xl font-bold  text-teal-900 mb-8">
          Lists For Your Selected Board
        </h1>
        {lists.map((list) => (
          <div
            key={list.id}
            className="bg-gradient-to-r from-yellow-300 to-orange-400 mt-4 p-4 rounded-lg shadow-md flex justify-between items-center text-black font-bold text-center hover:yellow-500 hover:to-cyan-400"
          >
            <Link to={`/view-list/${boardId}/${list.id}`}>{list.name}</Link>

            <Link to={`/view-list/${boardId}/${list.id}`}>
              {" "}
              <TbListDetails className="text-2xl hover:text-yellow-500" />
            </Link>
          </div>
        ))}
        <div className="mt-10">
          <div className="flex items-center space-x-4">
            <h4 className="mb-10 text-teal-950 text-xl font-bold mt-10 ">
              {" "}
            ...
            />
            <button
              className="px-6 py-3 text-black rounded font-semibold bg-gradient-to-r from-blue-300 to bg-yellow-400 hover:from-yellow-400 hover:to-yellow-300"
